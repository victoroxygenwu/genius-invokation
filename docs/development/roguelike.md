...
**默认关卡配置（FloorConfig[]）：**
```typescript
// data.ts 中的 ROGUELIKE_CONFIG，仅作为默认预设
[
...
在 `data.ts` 的 `DEFAULT_EVENTS` 数组中添加：

```typescript
{
  id: 2017,  // 唯一 ID，建议 2001-2999 范围
  name: "神秘旅人",
  imageUrl: "/events/2017_traveler.jpg",
...
3. `event-descriptors.ts` — `CONDITION_DESCRIPTORS` 或 `EFFECT_DESCRIPTORS`
